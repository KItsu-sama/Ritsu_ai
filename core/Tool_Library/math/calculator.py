...
"""
        ...
        if con['type'] == 'bound':
                ...
                if var not in bounds:
                    bounds[var] = [None, None]  # min, max
                if op == '<=':
                    if bounds[var][1] is None or value < bounds[var][1]:
                        bounds[var][1] = value
                elif op == '>=':
                    if bounds[var][0] is None or value > bounds[var][0]:
                        bounds[var][0] = value
                else:
                    raise ValueError(f"Unknown operator in bound: {op}")
            elif con['type'] == 'line':
                a, b = con['coeffs']
                op = con['op']
                c = con['value']
